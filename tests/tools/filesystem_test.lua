...
describe("Filesystem Module", function()
  ...
  after(function()
    ...
    if temp_test_dir then
      ...
      logger.debug("Cleaned up test directory")
    end
  end)
  ...
  describe("Core File Operations", function()
    ...
    it("should append to files", function()
      ...
      -- Append content
      local append_success, append_err = fs.append_to_file(file_path, append_content)
  ...
  describe("Path Operations", function()
    ...
    it("should extract file base names and extensions", function()
      local path = "/path/to/file.txt"
      local basename = fs.get_basename(path)
      ...
      local no_ext_basename = fs.get_basename(no_ext_path)
      ...
      local multi_basename = fs.get_basename(multi_dot_path)
      local multi_extension = fs.get_extension(multi_dot_path)
      
      expect(multi_basename).to.equal("archive.tar")
    ...
    it("should determine if paths are absolute", function()
      ...
      expect(fs.is_absolute_path(unix_abs)).to.be_truthy()
      
      -- Unix relative path
      local unix_rel = "relative/path"
      expect(fs.is_absolute_path(unix_rel)).to.be_falsy()
      
      -- Windows style absolute path (if supported)
      if fs.is_windows then
        local win_abs = "C:\\Windows\\Path"
        expect(fs.is_absolute_path(win_abs)).to.be_truthy()
      end
    end)
  end)
  ...
  describe("Information Functions", function()
    ...
    it("should retrieve file size correctly", function()
      ...
      local nonexistent_size, nonexistent_err = fs.get_file_size(test_dir_path .. "/nonexistent.txt")
      expect(nonexistent_size).to_not.exist()
      expect(nonexistent_err).to.exist()
    end)
    ...
    it("should retrieve file modification time", function()
      ...
      -- Test with non-existent file
      local nonexistent_time, nonexistent_err = fs.get_modified_time(test_dir_path .. "/nonexistent.txt")
      expect(nonexistent_time).to_not.exist()
      expect(nonexistent_err).to.exist()
    end)
    ...
    it("should handle error cases gracefully", { expect_error = true }, function()
      -- Test with invalid inputs
      local result1, err1 = fs.get_file_size(nil)
      expect(result1).to_not.exist()
      expect(err1).to.exist()
      expect(err1.context).to.exist()
      expect(err1.context.provided_type).to.equal("nil")
      
      -- Test with directory instead of file
      local result2, err2 = fs.get_file_size(test_dir_path)
      expect(result2).to_not.exist()
      expect(err2).to.exist()
      
      -- Test file operations on non-existent file
      local result3, err3 = fs.read_file(test_dir_path .. "/nonexistent.txt")
