--- @brief Coverage system tests
--- Tests the debug hook based coverage system, including:
--- - Coverage tracking and statistics
--- - Save and load functionality
--- - Error handling
--- - Integration with hooks
--- - Buffer management
--- @module tests.coverage.coverage_test
...
-- Helper to create a properly identified test string
...
describe("coverage", function()
  ...
  it("tracks basic line coverage", function()
    ...
    local code = create_test_string("basic_test", [[
      local x = 1
      if x > 0 then
        return "positive"
      else
  ...
  it("handles file move errors during atomic writes", function()
    ...
    local success, err = pcall(function() coverage.save_stats() end)
  ...
  it("respects include/exclude patterns", function()
    ...
    local included_code = create_test_string("include_test", [[
      local x = 1
      return x + 1
    ]])
    ...
    local excluded_code = create_test_string("exclude_test", [[
      local y = 2
      return y + 1
    ]])
